...
def test_prompt_addition_dry_run_normalizes_title_and_registered_skills(tmp_path):
    ...
    source.write_text(
        "Create a precise operational checklist using /visual-assets only when graphics are required.\n",
        encoding="utf-8",
    )
    ...
    assert (
        result.filename == f"{result.sequence:03d}_OPERATIONAL_CHECKLIST_AUTHORING.md"
    )
...
def test_prompt_addition_transaction_rolls_back_on_validation_failure(
    ...
    with pytest.raises(ValueError, match="forced validation failure"):
        add_prompt.add_prompt_transaction(
            ...
            approval_token=_approval_token(
                add_prompt, suite, source, "Rollback prompt"
            ),
        )
...
def test_prompt_addition_wrappers_and_platform_matrix_present():
    ...
    matrix = json.loads(
        (ROOT / "integrations/platform_tool_matrix.json").read_text(encoding="utf-8")
    )
...
def test_prompt_addition_writes_schema_valid_runtime_receipt(tmp_path, monkeypatch):
    ...
    source.write_text("Produce a bounded checklist.", encoding="utf-8")
    monkeypatch.setattr(
        add_prompt, "validate_staged_suite", lambda *args, **kwargs: None
    )
    result = add_prompt.add_prompt_transaction(
        ...
        run_full_tests=False,
        approval_token=_approval_token(
            add_prompt, suite, source, "Receipt validation prompt"
        ),
    )
...
def test_prompt_addition_receipt_failure_rolls_back_promoted_files(
    tmp_path, monkeypatch
):
    import add_prompt
    ...
    with pytest.raises(ValueError, match="forced receipt validation failure"):
        add_prompt.add_prompt_transaction(
            ...
            run_full_tests=False,
            approval_token=_approval_token(
                add_prompt, suite, source, "Receipt rollback prompt"
            ),
        )
    assert {p.name for p in (suite / "prompts").glob("*.md")} == before_prompts
...
def test_rebuild_updates_category_and_template_crosswalk_for_new_prompt(tmp_path):
    ...
    taxonomy = json.loads(
        (suite / "config/category_taxonomy.json").read_text(encoding="utf-8")
    )
    assert prepared.prompt_id in taxonomy["categories"]["enablement"]["prompt_ids"]
    crosswalk = json.loads(
        (suite / "integrations/template_to_prompt_crosswalk.json").read_text(
            encoding="utf-8"
        )
    )
    run_manifest = next(
        ...
        if row["template_id"] == "core/run-manifest"
    )
...
def test_md_cli_exposes_prompt_addition_dry_run(tmp_path):
    ...
    env.update({"MD_NO_TUI": "1", "MD_LOG_DIR": str(tmp_path / "logs")})
    proc = subprocess.run(
        [
            sys.executable,
...
def test_prompt_addition_rejects_duplicate_title_and_slug(tmp_path):
    ...
    source = tmp_path / "raw.md"
    source.write_text(
        "A distinct body that must not reuse an existing identity.\n", encoding="utf-8"
    )
...
def test_prompt_addition_infers_known_prompt_template_and_skill_references(tmp_path):
    ...
    prepared = add_prompt.prepare_prompt(
        ROOT, source=source, title="Reference inference prompt"
    )
...
def test_prompt_addition_persists_schema_valid_runtime_receipt(tmp_path, monkeypatch):
    ...
    result = add_prompt.add_prompt_transaction(
        suite,
        source=source,
        title="Receipt persistence prompt",
        run_full_tests=False,
        approval_token=_approval_token(
            add_prompt, suite, source, "Receipt persistence prompt"
        ),
    )
    receipt = suite / result["receipt_path"]
    assert receipt.is_file()
    ...
    schema = json.loads(
        (suite / "schemas/prompt_addition_receipt.schema.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert (
        ...
        == (suite / "VERSION").read_text(encoding="utf-8").strip()
    )
...
def test_prompt_transaction_reads_source_once_before_staging(tmp_path, monkeypatch):
    ...
    token = _approval_token(
        add_prompt, suite, source, "Immutable source snapshot prompt"
    )
    calls = (
        0  # the execution transaction must capture one immutable snapshot of its own
    )
    add_prompt.add_prompt_transaction(
        suite,
        source=source,
...
def test_prompt_addition_xml_escapes_imported_source_and_binds_digest(tmp_path):
    ...
    prepared = add_prompt.prepare_prompt(
        ROOT, source=source, title="Source quarantine prompt"
    )
...
def test_prompt_addition_uses_one_immutable_source_snapshot(tmp_path, monkeypatch):
    ...
    def changing_prepare(*args, **kwargs):
        ...
        if calls["count"] == 1:
            source.write_text(
                "Changed prompt body after initial read.\n", encoding="utf-8"
            )
        return result
...
def test_prompt_addition_parses_only_valid_leading_frontmatter(tmp_path):
    ...
    prepared = add_prompt.prepare_prompt(
        ROOT, source=source, title="Frontmatter parsing prompt"
    )
...
def test_prompt_addition_fails_closed_when_staged_identity_differs_from_approved_preview(
    tmp_path, monkeypatch
):
    ...
    def verify_then_simulate_concurrent_writer(root, prepared, packs, token):
        ...
        concurrent_path = (
            f"prompts/{concurrent_sequence:03d}_CONCURRENT_WRITER_PROMPT.md"
        )
        ...
        shutil.copy2(
            root / catalog["prompts"][-1]["canonical_path"], root / concurrent_path
        )
    ...
    monkeypatch.setattr(
        add_prompt, "_verify_approval_token", verify_then_simulate_concurrent_writer
    )
    monkeypatch.setattr(
        add_prompt, "validate_staged_suite", lambda *args, **kwargs: None
    )
