...
STORE.update(
    {
        f"file://{(ROOT / 'schemas' / name).resolve()}": schema
...
def test_every_schema_has_positive_and_adversarial_contract_fixtures():
    ...
    for name in sorted(SCHEMAS):
        stem = Path(name).stem.replace(".schema", "")
        folder = ROOT / "evaluations/schema_contracts" / stem
        healthy = json.loads((folder / "healthy.json").read_text())
        adversarial = json.loads((folder / "adversarial.json").read_text())
        valid_errors = list(validator(name).iter_errors(healthy))
        invalid_errors = list(validator(name).iter_errors(adversarial))
        if valid_errors or not invalid_errors:
            failures.append(
                {
                    "schema": name,
                    "healthy_errors": [x.message for x in valid_errors[:3]],
                    "adversarial_was_rejected": bool(invalid_errors),
                }
            )
...
def test_prompt_frontmatter_schema_accepts_catalog_entries():
    ...
    schema = json.loads((ROOT / "config/prompt-frontmatter.schema.json").read_text())
    check = Draft202012Validator(schema)
    failures = []
    for p in (ROOT / "prompts").glob("*.md"):
        _, fm, _ = p.read_text(encoding="utf-8").split("---", 2)
        issues = list(check.iter_errors(yaml.safe_load(fm)))
        if issues:
            failures.append((p.name, [x.message for x in issues[:3]]))
    assert not failures, failures
...
def test_current_runtime_artifacts_validate_against_declared_schemas():
    ...
    for name in [
        ...
        mappings.extend(
            json.loads((ROOT / "integrations" / name).read_text()).get("mappings", [])
        )
    check("cross_catalog_mapping.schema.json", mappings)
    check(
        "scenario_fixture.schema.json",
        [
            json.loads(p.read_text())
            for p in (ROOT / "evaluations/scenarios").glob("C-*/*.json")
        ],
    )
    check(
        "pair_adversarial_fixture.schema.json",
        [
            json.loads(p.read_text())
            for p in (ROOT / "evaluations/pairs").glob("*/adversarial.json")
        ],
    )
    check(
        "skill_conformance.schema.json",
        [
            json.loads(p.read_text())
            for p in (ROOT / "evaluations/skills").glob("*.json")
        ],
    )
    assert not failures, failures
