...
def test_skill_id_validation_rejects_path_traversal_and_unsafe_names():
    ...
    assert (
        security.validate_identifier("visual-assets", kind="skill") == "visual-assets"
    )
...
def test_installer_rejects_runtime_symlink_escape(tmp_path):
    installer = load(ROOT / "tools/install.py", "md_install_symlink")
...
def test_installer_refuses_project_inside_source_tree(tmp_path):
    installer = load(ROOT / "tools/install.py", "md_install_overlap")
...
def test_runtime_receipts_do_not_mutate_sealed_release_by_default():
    ...
    inventory = (ROOT / "tools" / "sync_installed_skills.py").read_text(
        encoding="utf-8"
    )
...
def test_telemetry_rejects_stringified_key_collisions_cycles_and_oversize(tmp_path):
    ...
    with pytest.raises(ValueError, match="key collision"):
        telemetry.append_event(
            "security", "collision", context={1: "a", "1": "b"}, log_dir=tmp_path
        )
    cyclic = {}
    cyclic["self"] = cyclic
    ...
    with pytest.raises(ValueError, match="text exceeds"):
        telemetry.append_event(
            "security", "large", context={"value": "x" * 100_001}, log_dir=tmp_path
        )
...
def test_github_repository_validation_rejects_unsafe_transports_and_urls():
    ...
    assert (
        security.validate_github_repository("https://github.com/owner/repo")
...
def test_local_skill_registration_rolls_back_when_staging_fails(tmp_path, monkeypatch):
    module = load(
        ROOT / "tools" / "register_local_skill_dual.py", "register_local_rollback"
    )
    ...
    monkeypatch.setattr(
        module, "append_event", lambda *args, **kwargs: {"log_file": "test"}
    )
...
def test_remote_skill_install_rolls_back_all_destinations_on_cli_failure(
...
def test_installer_rollback_preserves_non_utf8_agent_file(tmp_path, monkeypatch):
    installer = load(ROOT / "tools/install.py", "md_install_non_utf8")
    project = tmp_path / "project"
    project.mkdir()
...
def test_guidance_sync_is_transactional_and_receipt_path_is_project_relative(
    tmp_path, monkeypatch
):
    ...
    result = sync.sync_guidance(
        project, ROOT, receipt_path=".prompt_suite/receipt.json"
    )
    saved = json.loads(
        (project / ".prompt_suite/receipt.json").read_text(encoding="utf-8")
    )
    assert (
        saved["receipt_path"]
        == (project / ".prompt_suite/receipt.json").absolute().as_posix()
    )
    assert result["receipt_path"] == saved["receipt_path"]


def test_local_skill_receipt_failure_restores_previous_installations(
    tmp_path, monkeypatch
):
    module = load(
        ROOT / "tools" / "register_local_skill_dual.py", "register_receipt_rollback"
    )
    source = tmp_path / "source"
    source.mkdir()
    (source / "SKILL.md").write_text("---\nname: demo-skill\n---\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        module,
        "atomic_write_json",
        lambda *args, **kwargs: (_ for _ in ()).throw(OSError("receipt failed")),
    )
    monkeypatch.setattr(
        sys,
        "argv",
...
def test_skill_conformance_requires_exact_relative_artifact_path(tmp_path, monkeypatch):
    module = load(
        ROOT / "tools" / "run_skill_conformance.py", "skill_conformance_exact_path"
    )
            ...
            "expected_artifacts": [
                {
                    "prompt_id": "MD-01",
                    "primary": {
                        "path": "reports/expected/result.md",
                        "format": "markdown",
...
def test_locked_skill_source_verifies_recorded_tarball_hash(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        module, "github_tarball_sha256", lambda repository, commit: "c" * 64
    )
    with pytest.raises(ValueError, match="tarball SHA-256"):
        module._locked_source(lock, "demo-skill", "https://github.com/owner/repo")


def test_remote_skill_cli_runs_in_isolated_home_then_promotes_transactionally(
    tmp_path, monkeypatch
):
    module = load(ROOT / "tools" / "skill_dual.py", "skill_cli_isolation")
    roots = {
        "agents": tmp_path / "live-agents",
        "claude-code": tmp_path / "live-claude",
        "opencode": tmp_path / "live-opencode",
    }
    monkeypatch.setattr(module, "all_default_destinations", lambda: roots)
    monkeypatch.setattr(
        module, "append_event", lambda *args, **kwargs: {"log_file": "test"}
...
def test_remote_skill_receipt_failure_restores_previous_installations(
    tmp_path, monkeypatch
):
