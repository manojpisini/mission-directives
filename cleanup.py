...
class PurgeIncompleteError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        state = "rollback remains safe" if rollback_safe else "cleanup is committed; partial restoration is forbidden"
        super().__init__(
...
def _project_path(project: Path) -> Path:
    ...
    if candidate == ensure_no_symlink_components(SOURCE):
        raise ValueError("Project path must identify the installed project, not the distribution root")
    return candidate
...
def _validate_installed_suite(prompts: Path) -> tuple[str, str]:
    ...
    for path in (version_path, release_path):
        if not path.is_file():
            raise ValueError(f"Installed suite is missing required identity file: {path}")
    version = version_path.read_text(encoding="utf-8").strip()
    release_id = release_path.read_text(encoding="utf-8").strip()
    if release_id != f"mission-directives-{version}":
        raise ValueError("Installed prompts directory does not have a consistent Mission Directives identity")
...
def _load_installation_receipt(project: Path) -> dict:
    ...
    if not isinstance(data, dict) or data.get("suite_destination") != "prompts":
        raise ValueError("Installation receipt is malformed or does not describe ./prompts")
    return data
...
def _valid_marker(path: Path) -> bool:
    ...
    try:
        ...
        return False
    return isinstance(data, dict) and data.get("created_by") == "mission-directives" and data.get("path") == path.name
...
def _directory_empty_or_marker_only(path: Path) -> bool:
    ...
    return not entries or all(entry.name == MANAGED_MARKER and entry.is_file() for entry in entries)
...
def _build_plan(project: Path) -> dict:
    ...
    file_states = {name: _hash_file_state(safe_child(project, name)) for name in INTEGRATION_FILES}
    removal_states = {}
    for name in sorted(set(planned_removals)):
        path = safe_child(project, name)
        if path.exists():
            removal_states[name] = {"exists": True, "tree_state_sha256": _tree_state_digest(path), "bytes": _tree_size(path)}
        else:
    ...
    plan = {
        ...
        "managed_file_results": {name: (None if value is None else hashlib.sha256(value.encode("utf-8")).hexdigest()) for name, value in updates.items()},
...
def preview_cleanup(project: Path, progress: Callable[[str], None] | None = None) -> dict:
    project = _project_path(project)
    if progress:
...
def _write_managed_updates(project: Path, updates: dict[str, str | None]) -> tuple[list[str], list[str]]:
    changed: list[str] = []
    deleted: list[str] = []
...
def _quarantine_paths(
    ...
    for index, name in enumerate(removals):
        ...
        if source.is_symlink() or not source.is_dir():
            raise ValueError(f"Refusing to clean non-directory or symlink path: {source}")
        ...
        for kind, names in ((b"D", dirnames), (b"F", filenames)):
            for name in names:
                ...
                if kind == b"D" and not candidate.is_dir():
                    raise ValueError(f"Refusing non-directory entry in managed tree: {candidate}")
                if kind == b"F" and not candidate.is_file():
                    raise ValueError(f"Refusing non-regular file in managed tree: {candidate}")
                st = candidate.stat()
...
def _purge_quarantine(project: Path, moved: list[tuple[Path, Path]], staging: Path) -> list[str]:
    ...
    for original, quarantined in moved:
        try:
            ...
            if quarantined.exists():
                try:
                    destructive_change = destructive_change or _tree_state_digest(quarantined) != before[quarantined]
                except Exception:
...
def cleanup(
    ...
    *,
    ...
    staging = safe_child(project, f".md-cleanup-staging-{os.getpid()}-{uuid.uuid4().hex}")
    ...
    try:
        with exclusive_lock(lock):
            ...
            if not hmac.compare_digest(expected, approval_token):
                raise ValueError("Project state changed since approval; generate a new cleanup preview and retry")
            if progress:
                progress("verified approval against current project state")
            snapshots = {name: _regular_file_snapshot(safe_child(project, name)) for name in INTEGRATION_FILES}
            updates, _ = _managed_file_updates(project)
            changed_files, deleted_files = _write_managed_updates(project, updates)
            if progress:
                progress("removed managed AGENTS.md, CLAUDE.md, and .gitignore sections")

            if docs_marker.exists() and "docs" not in plan["planned_removals"]:
                docs_marker_snapshot = _regular_file_snapshot(docs_marker)
                docs_marker.unlink()
            reclaimable = sum(_tree_size(safe_child(project, name)) for name in plan["planned_removals"] if safe_child(project, name).exists())
            ...
            result = {
                ...
                "project_root": project.as_posix(),
                "suite_version": plan["suite_version"],
                "removed_paths": [path.relative_to(project).as_posix() for path, _ in moved],
                ...
                if is_within(receipt, project):
                    for name in plan["planned_removals"]:
                        ...
                        if receipt == target or is_within(receipt, target):
                            raise ValueError("Cleanup receipt must not be written inside a removed path")
            if progress:
            ...
            try:
                ...
                raise
            moved.clear()
            result["duration_ms"] = max(0, int((dt.datetime.now(dt.timezone.utc) - started).total_seconds() * 1000))
            ...
            if receipt is not None:
                try:
                    ...
                    result["warnings"] = sorted(set(result["warnings"] + [f"receipt_write_failed:{receipt_error}"]))
            if progress:
...
def main() -> int:
    ...
    parser.add_argument("--yes", action="store_true", help="Confirm the current preview noninteractively")
    parser.add_argument("--approval-token", help="Approval token returned by --dry-run")
    parser.add_argument("--receipt", type=Path, help="Optional cleanup receipt path outside removed paths")
    parser.add_argument("--no-tui", action="store_true")
    args = parser.parse_args()

    tui = TUI("Mission Directives cleanup", total=10, enabled=False if args.no_tui else None)
    tui.start()
    ...
    try:
        ...
        if not args.yes and not args.approval_token:
            if not sys.stdin.isatty():
                raise ValueError("Explicit confirmation is required; rerun with --yes or a dry-run approval token")
            if not _confirm(preview):
                raise UserDeclined
        result = cleanup(Path(raw), approval_token=token, receipt_path=args.receipt, progress=tui.step)
    except UserDeclined:
        tui.finish("DECLINED")
        tui.summary("DECLINED", "Mission Directives cleanup was declined", [("Project", raw)])
        print(json.dumps({"status": "declined", "project_root": raw}, indent=2), file=sys.stderr)
        return 3
    except KeyboardInterrupt:
        tui.finish("CANCELLED")
        tui.summary("CANCELLED", "Mission Directives cleanup was interrupted", [("Project", raw)])
        print(json.dumps({"status": "cancelled", "project_root": raw}, indent=2), file=sys.stderr)
        return 130
        ...
        tui.summary(
            ...
            [
                ...
                ("Recovery", "Release file locks, then remove only the reported quarantine directory"),
            ],
        )
        print(
            json.dumps(
        ...
        tui.summary(
            ...
            [("Project", raw), ("Reason", str(exc)), ("Recovery", "Project state restored when mutation had begun")],
        )
