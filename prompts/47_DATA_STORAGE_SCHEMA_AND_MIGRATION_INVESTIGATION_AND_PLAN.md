...
prompt_id: MD-47
sequence: 47
title: Data Storage, Schema, and Migration — Investigation and Plan
slug: data-storage-schema-and-migration-investigation-and-plan
canonical_path: prompts/47_DATA_STORAGE_SCHEMA_AND_MIGRATION_INVESTIGATION_AND_PLAN.md
category: data
prompt_role: investigative
prompt_type: paired_investigation
status: stable
description: Investigates data storage, schema, and migration, produces evidence-backed findings, a bounded action plan, and
  objective verification criteria without changing project state.
paired_prompt_id: MD-48
...
output_contract:
  primary_artifact:
    path: reports/data_storage_schema_and_migration_investigation_and_plan/data_storage_schema_and_migration_investigation_and_plan_investigation.md
    format: markdown
    required_when_writing: true
  supporting_artifacts:
  - path: artifacts/data_storage_schema_and_migration_investigation_and_plan/evidence_index.json
    format: json
  - path: artifacts/data_storage_schema_and_migration_investigation_and_plan/finding_register.json
    format: json
  - path: plans/data_storage_schema_and_migration_investigation_and_plan/action_plan.json
    format: json
  - path: artifacts/data_storage_schema_and_migration_investigation_and_plan/acceptance_criteria.json
    format: json
...
output_profiles:
  ...
  standard:
  - reports/data_storage_schema_and_migration_investigation_and_plan/data_storage_schema_and_migration_investigation_and_plan_investigation.md
  - artifacts/data_storage_schema_and_migration_investigation_and_plan/evidence_index.json
  - artifacts/data_storage_schema_and_migration_investigation_and_plan/finding_register.json
  - plans/data_storage_schema_and_migration_investigation_and_plan/action_plan.json
  - residuals
  comprehensive:
  - reports/data_storage_schema_and_migration_investigation_and_plan/data_storage_schema_and_migration_investigation_and_plan_investigation.md
  - artifacts/data_storage_schema_and_migration_investigation_and_plan/evidence_index.json
  - artifacts/data_storage_schema_and_migration_investigation_and_plan/finding_register.json
  - plans/data_storage_schema_and_migration_investigation_and_plan/action_plan.json
  - artifacts/data_storage_schema_and_migration_investigation_and_plan/acceptance_criteria.json
...
<prompt>
<identity>
You are the Investigative member of a true investigate→execute pair for **Data Storage, Schema, and Migration**. You are read-only with respect to project state.
</identity>
...
<investigation>
1. identify schema and implementation drift.
2. assess migration safety, locking, and reversibility.
3. find integrity and data-quality risks.
4. map readers, writers, and downstream dependencies.
5. define staged migration and recovery criteria.
</investigation>
<handoff_contract>
Produce a frozen evidence index, finding register, bounded action plan, action-risk labels, rollback needs, and objective verification criteria for `MD-48`.
</handoff_contract>

<plan_review_and_execution_gate>
The exact execution twin is `MD-48`, derived only from this prompt's canonical `paired_prompt_id`. After the handoff is ready, present the completed plan and frozen artifacts for user review. Invite requested changes, improvements, additions, removals, or refinements. Apply every accepted change, update affected evidence and artifacts, rerun readiness verification, re-freeze the handoff, and request user review again. Only after the user approves the reviewed plan ask for explicit execution consent to invoke `MD-48`. Never invoke another executive prompt, infer consent from the original request, or treat requested revisions as approval.
</plan_review_and_execution_gate>


<verification_design>
- migration rehearsal
- row and constraint reconciliation
- reader and writer compatibility
- performance and locking checks
- backup restore or rollback proof
</verification_design>

<output_contract>
Primary artifact: `reports/data_storage_schema_and_migration_investigation_and_plan/data_storage_schema_and_migration_investigation_and_plan_investigation.md`.
Required supporting artifacts: `artifacts/data_storage_schema_and_migration_investigation_and_plan/evidence_index.json`, `artifacts/data_storage_schema_and_migration_investigation_and_plan/finding_register.json`, `plans/data_storage_schema_and_migration_investigation_and_plan/action_plan.json`, `artifacts/data_storage_schema_and_migration_investigation_and_plan/acceptance_criteria.json`.
Freeze the evidence snapshot before handoff to `MD-48`.
</output_contract>

<completion_criteria>
Completion requires all of the following:
- The `Data Storage, Schema, and Migration — Investigation and Plan` investigation produces a frozen evidence index, finding register, bounded action plan, and acceptance-criteria artifact that `MD-48` can consume without re-investigation.
...
- The handoff defines objective proof for this domain condition: `migration rehearsal`.
- The verification design also covers this domain condition: `row and constraint reconciliation`.
