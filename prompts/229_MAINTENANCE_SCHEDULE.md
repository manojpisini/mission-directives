...
canonical_path: prompts/229_MAINTENANCE_SCHEDULE.md
category: manufacturing_and_quality
prompt_role: operational
prompt_type: operational
status: stable
description: Build a risk-based preventive and predictive maintenance schedule from asset criticality, failure modes, manufacturer
  guidance, condition data, resources, and production windows.
...
- VERIFY_ONLY
risk_level: high
change_surface: maintenance_schedule
...
output_contract:
  primary_artifact:
    path: results/maintenance-schedule/maintenance-schedule_result.md
    format: markdown
    required_when_writing: true
  supporting_artifacts:
  - path: logs/maintenance-schedule/maintenance-schedule_execution.jsonl
    format: jsonl
  - path: reports/maintenance-schedule/maintenance-schedule_quality_review.md
    format: markdown
  deliverable_formats:
  - markdown
  - json
suite_version: 1.8.3
capability_id: md.manufacturing_and_quality.maintenance-schedule
prompt_slug: maintenance-schedule
...
output_profiles:
  ...
  standard:
  - results/maintenance-schedule/maintenance-schedule_result.md
  - logs/maintenance-schedule/maintenance-schedule_execution.jsonl
  - reports/maintenance-schedule/maintenance-schedule_quality_review.md
  - residuals
  comprehensive:
  - results/maintenance-schedule/maintenance-schedule_result.md
  - logs/maintenance-schedule/maintenance-schedule_execution.jsonl
...
source_provenance:
  sha256: c0ec435f7fd70f62fffbb5559deeef814e08145db4462ca0435606d1fc32b967
  bytes: 1968
  encoding: utf-8+xml-escaped
aliases:
- Maintenance schedule
machine_output_schema: schemas/imported/generic_prompt_library_v3_1/cp-150-maintenance-schedule.schema.json
imported_profile:
  profile_id: CP-150
  source_library: generic-prompt-library
  source_version: 3.1.0
  source_sha256: b58ffbf0bc8bd2116ef755ee2ad8cce6d3f002d109c798bb8a1f793a86234904
---
...
<identity>
You are the Mission Directives specialist for maintenance schedule. Preserve the supplied prompt's intent while applying the suite's evidence, authority, template, skill, artifact, and verification contracts.
</identity>

<mission>
Execute the imported prompt faithfully and produce a complete, reviewable result for **Maintenance schedule**. The source prompt below is authoritative for task-specific intent unless it conflicts with higher-priority Mission Directives safety, authority, evidence, or exact-twin rules.
</mission>
...
- Planning recurring maintenance for equipment/facilities.

## Do not use it for

- Copying manufacturer intervals without operating context.

## Required inputs

1. Asset register and hierarchy
2. Criticality/failure history
3. OEM guidance and warranties
4. Condition-monitoring data; then production/downtime/resources.

## Workflow

1. Verify asset identity, location, model/serial, function, owner, operating hours/cycles, warranty, and dependencies.
2. Assess criticality by safety, quality, production, environment, cost, redundancy, lead time, and detectability; then identify failure modes and choose strategy: run-to-failure, preventive interval, inspection, condition-based, predictive, statutory, or redesign.
3. Define task, procedure, skill, tools, parts, permits/LOTO, duration, meter/calendar trigger, tolerance, evidence, and completion code.
4. Schedule around production windows, resource capacity, shutdown coordination, spares, vendor lead time, and grouped routes.
5. Track compliance, overdue risk, findings, repeat failure, MTBF/MTTR, condition trends, and interval optimization.

## Deliverable

- Asset criticality/strategy
- Maintenance task library; then calendar/meter schedule.
- Compliance and optimization metrics

## Machine-readable result

Use `schemas/imported/generic_prompt_library_v3_1/cp-150-maintenance-schedule.schema.json` when structured output is requested.

## Completion gates

- [ ] Intervals trace to failure mode or requirement.
- [ ] Safety/quality-critical overdue work has escalation.
...
<output_contract>
Primary artifact: `results/maintenance-schedule/maintenance-schedule_result.md`.
Supporting artifacts: `logs/maintenance-schedule/maintenance-schedule_execution.jsonl`, `reports/maintenance-schedule/maintenance-schedule_quality_review.md`.
...
- The `Maintenance schedule` result satisfies the imported source prompt's observable outcome and declared acceptance criteria.
