...
parser.add_argument('--publish',action='store_true',help='Write canonical root EVALUATION_STATUS.json during release sealing.')
args=parser.parse_args()
errors=[]
cat=json.loads((ROOT/'catalog.json').read_text()); sc=json.loads((ROOT/'SCENARIO_CATALOG.json').read_text())
pairs=[p for p in cat['prompts'] if p['prompt_role']=='investigative' and p.get('paired_prompt_id')]
...
conformance_required=installable+[s for s in registered_skills if s.get('kind')=='local_installed' and s.get('auto_select_allowed')]
fixture_schema=json.loads((ROOT/'schemas/scenario_fixture.schema.json').read_text())
pair_schema=json.loads((ROOT/'schemas/pair_adversarial_fixture.schema.json').read_text())
...
if len(template_fixture_files)!=json.loads((ROOT/'template_registry.json').read_text())['template_count']*3: errors.append('template evaluation coverage incomplete')
if len(schema_contract_files)!=len(schema_files)*2: errors.append(f'schema contract coverage {len(schema_contract_files)} != {len(schema_files)*2}')
if len(prompt_files)!=len(cat['prompts'])*3: errors.append(f'prompt fixture count {len(prompt_files)} != {len(cat["prompts"])*3}')
if len(scenario_files)!=len(sc['composite_scenarios'])*3: errors.append('scenario fixture coverage incomplete')
if len(pair_files)!=len(pairs): errors.append('pair adversarial coverage incomplete')
if len(skill_files)!=len(conformance_required): errors.append(f'skill conformance coverage {len(skill_files)} != {len(conformance_required)}')
if len(pvs_files)!=len(pairs): errors.append('pair-vs-single coverage incomplete')
for p in prompt_files+scenario_files:
    d=load(p)
    if d:
        try: js_validate(d,fixture_schema)
        except Exception as e: errors.append(f'{p}: fixture schema {e}')
for p in pair_files:
    d=load(p)
    if d:
        try: js_validate(d,pair_schema)
        except Exception as e: errors.append(f'{p}: pair schema {e}')
for p in skill_files:
    d=load(p)
    if d:
        try: js_validate(d,skill_schema)
        except Exception as e: errors.append(f'{p}: skill schema {e}')
...
if missing_mutations: errors.append(f'missing prompt-body mutation definitions: {sorted(missing_mutations)}')
...
for case in load(ROOT/'evaluations/negative_routing.json')['cases']:
    ...
    if bad: errors.append(f"negative route {case['case_id']} selected {sorted(bad)}")
# Recovery drill transitions must be legal.
drill=load(ROOT/'evaluations/recovery_drills/failed_execution_rollback_residual.json')
cur=drill['initial_state']
for nxt in drill['transitions']:
    if nxt not in md.STATE['transitions'].get(cur,[]): errors.append(f'illegal recovery drill transition {cur}->{nxt}')
    cur=nxt
profiles=json.loads((ROOT/'model_profiles.json').read_text())['profiles']
measured=[p for p in profiles if p.get('measurement_status')=='measured' and p.get('production_eligible')]
live_skill_pass=sum(1 for p in skill_files if isinstance(load(p),dict) and load(p).get('live_status')=='pass')
status={
 ...
 'machine_reference_runs':len(list((ROOT/'evaluations/golden_runs').glob('C-*/manifest.json'))),
 'limitations':['No external language-model benchmark was run in this build environment.','No third-party skill was installed or executed.','Reference runs are machine-generated contracts, not human-reviewed golden outputs.'],
 'errors':errors
}
target=ROOT/'EVALUATION_STATUS.json' if args.publish else safe_child(ROOT,RUNTIME_RESULT_REL)
