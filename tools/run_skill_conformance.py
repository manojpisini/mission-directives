...
try:
    from security_utils import atomic_write_json, ensure_no_symlink_components, exclusive_lock, read_json_bounded, safe_child, validate_identifier
except ImportError:
    from tools.security_utils import atomic_write_json, ensure_no_symlink_components, exclusive_lock, read_json_bounded, safe_child, validate_identifier
...
def main() -> int:
    ...
    locks = {row['skill_id']: row for row in read_json_bounded(ROOT / 'skills.lock.json')['entries']}
    lock = locks.get(skill_id)
    errors: list[str] = []
    if spec.get('lock_required', True) and (not lock or lock.get('lock_status') != 'resolved'):
    ...
    if not out.is_dir():
        ...
        for route in spec['expected_contract'].get('expected_artifacts', []):
            ...
            if not a.allow_missing_optional:
                artifacts.extend(('supporting', item) for item in route.get('supporting', []))
            for role, artifact in artifacts:
                ...
                if not candidate.is_file():
                    errors.append(f'missing {role} artifact for {route["prompt_id"]}: {artifact["path"]}')
                    continue
                try:
                    ...
                    if fmt == 'json':
                        ...
                        errors.append(f'empty {role} artifact for {route["prompt_id"]}: {artifact["path"]}')
                except Exception as exc:
                    errors.append(f'invalid {artifact.get("format", "artifact")} {candidate}: {exc}')
    ...
    if a.publish:
        lock_path = safe_child(ROOT, f'.prompt_suite/locks/skill-conformance-{skill_id}.lock')
    ...
    target = ensure_no_symlink_components(Path(a.result_path).expanduser()) if a.result_path else safe_child(ROOT, f'.prompt_suite/runtime/skill-conformance/{skill_id}.json')
