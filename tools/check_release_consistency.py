...
def check(root: Path = ROOT) -> dict:
    ...
    for name in ("AGENTS.md", "CLAUDE.md"):
        ...
        if f"Mission Directives **{version}**" not in text:
            errors.append(f"{name}: managed guidance version is stale")
    import yaml

    for path in (root / "prompts").glob("*.md"):
        _, frontmatter, _ = path.read_text(encoding="utf-8").split("---", 2)
        if yaml.safe_load(frontmatter).get("suite_version") != version:
            errors.append(f"{path.relative_to(root)}: suite_version mismatch")
    ...
    for path in root.rglob("*.json"):
        ...
        if (
            ...
            and "suite_version" in value
            and value.get("suite_version") != version
        ):
            active_json_mismatches.append(rel.as_posix())
    if active_json_mismatches:
        errors.append(
            f"active JSON suite_version mismatches: {active_json_mismatches[:30]}"
        )
    runtime_lock_files = [
    ...
    if runtime_lock_files:
        errors.append(
            f"distribution contains runtime lock files: {runtime_lock_files[:30]}"
        )
    personal = re.compile(
    ...
    for path in root.rglob("*"):
        if (
            ...
            or any(x in path.parts for x in EXCLUDED_PARTS)
        ):
            continue
        rel = path.relative_to(root)
        if (
            ...
            or rel.parts[:2] == (".prompt_suite", "logs")
            or rel.parts[:2] == ("evaluations", "schema_contracts")
            or rel.parts[:1] == ("tests",)
        ):
            continue
        if personal.search(path.read_text(encoding="utf-8", errors="ignore")):
            personal_hits.append(rel.as_posix())
    if personal_hits:
        errors.append(f"personal absolute paths: {personal_hits[:20]}")
    if (root / ".prompt_suite" / "agent-guidance-receipt.json").exists():
        errors.append("distribution contains machine-local agent guidance receipt")
    return {
