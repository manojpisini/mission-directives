...
try:
    from security_utils import atomic_write_bytes, atomic_write_text, ensure_no_symlink_components, safe_child, is_within
except ImportError:
    from tools.security_utils import atomic_write_bytes, atomic_write_text, ensure_no_symlink_components, safe_child, is_within
...
_POLICY = load_policy()
DEFAULT_AGENT_FILES = list(_POLICY.get("default_agent_files") or FALLBACK_DEFAULT_AGENT_FILES)
...
def _validate_agent_files(agent_files: Iterable[str]) -> list[str]:
    ...
    for raw in agent_files:
        ...
        if not normalized or candidate.is_absolute() or ".." in candidate.parts:
            raise ValueError(f"Agent file must be a safe project-relative path: {raw!r}")
...
def render_guidance(project_root: Path | str, suite_root: Path | str = SUITE_ROOT, agent_file: str = "AGENTS.md") -> str:
    ...
    version = (suite_root / "VERSION").read_text(encoding="utf-8").strip() if (suite_root / "VERSION").exists() else "unknown"
    ...
    cleanup_py = _quote_command_path(_join(suite_rel, "cleanup.py"))
...
def _merge_block(existing: str, block: str) -> str:
    ...
    separator = "" if clean.endswith(newline * 2) else newline if clean.endswith(newline) else newline * 2
...
def sync_guidance(
    ...
    *,
    ...
    files = _validate_agent_files(agent_files or load_policy(suite_root).get("default_agent_files") or DEFAULT_AGENT_FILES)
    ...
    for name, path in target_paths.items():
        ...
        if remove:
            ...
            desired = _merge_block(existing, render_guidance(project_root, suite_root, name))
        ...
        if show_diff:
            diffs[name] = "".join(
                difflib.unified_diff(
                    existing.splitlines(keepends=True), desired.splitlines(keepends=True),
                    fromfile=f"a/{name}", tofile=f"b/{name}",
                )
            )

    status = ("dry_run_changes" if changed else "dry_run_unchanged") if dry_run else ("changed" if changed else "unchanged")
    receipt = {
        "schema_version": "1.0",
        "status": status,
        ...
        "suite_version": (suite_root / "VERSION").read_text(encoding="utf-8").strip() if (suite_root / "VERSION").exists() else "unknown",
        "managed_files": files,
...
def main() -> int:
    ...
    parser.add_argument("--receipt", default=".prompt_suite/agent-guidance-receipt.json")
    ...
    try:
        if args.print_block:
            print(render_guidance(args.project_root, suite_root, (files or DEFAULT_AGENT_FILES)[0]))
            return 0
