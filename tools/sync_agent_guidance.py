...
def render_guidance(
    ...
    blocks = [
        ...
        "",
        "```bash",
        f'python {md_py} route "<full user request>"',
        f'python {md_py} lookup "<operator discovery terms>" --limit 8',
        f"python {md_py} compare <TARGET_A> <TARGET_B>",
        ...
        "",
        f'1. Run `python {md_py} route "<full user request>"`.',
        f"2. If needed, compare close candidates with `python {md_py} compare <targets...>`.",
        f"3. Inspect every selected target with `python {md_py} explain <target>`.",
        f"4. Use `{catalog_path}` for prompt metadata and `{scenarios_path}` for composite workflows.",
        f"5. Use `{execution_path}` for phase order, modes, branches, locks, and completion semantics.",
        f"6. Load only the selected bodies from `{prompts_path}` plus their declared prerequisites.",
        f"7. Consult schemas, policies, `{skill_registry_path}`, `{policy_path}`, and `{loop_policy_path}` only when triggered.",
        "",
