...
def main() -> int:
    ...
    parser.add_argument('--publish', action='store_true', help='Write the canonical root TEST_RESULTS.json during a controlled release build.')
    parser.add_argument('--per-file-timeout', type=int, default=180, help='Maximum seconds allowed for each test file.')
    ...
    with tempfile.TemporaryDirectory(prefix='md-test-logs-') as log_dir:
        ...
        for test_file in test_files:
            ...
            try:
                ...
                proc = subprocess.run(
                    [sys.executable, '-m', 'pytest', '-q', relative, f'--junitxml={junit_path}'],
                    cwd=ROOT,
                    text=True,
                    capture_output=True,
                ...
                try:
                    ...
                    failures.append({'test_file': relative, 'junit_error': str(exc)})
                if proc.returncode != 0:
                    failures.append({'test_file': relative, 'return_code': proc.returncode})
            except subprocess.TimeoutExpired:
                outputs.append(f'## {relative}\nTIMEOUT after {args.per_file_timeout} seconds')
                failures.append({'test_file': relative, 'timeout_seconds': args.per_file_timeout})
    ...
    status = {
        ...
        'failures': failures,
        'limitations': ['Unit and deterministic runtime tests only; no external model or live skill execution.'],
    }
    target = ROOT / 'TEST_RESULTS.json' if args.publish else safe_child(ROOT, RUNTIME_RESULT_REL)
