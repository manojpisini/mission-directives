...
category_path=ROOT/'category_taxonomy.json'
category_data={'categories':{}}
for meta in rows:
    entry=category_data['categories'].setdefault(meta['category'],{'prompt_ids':[],'roles':[],'evidence_lanes':[],'media':[]})
    entry['prompt_ids'].append(meta['prompt_id'])
    if meta.get('prompt_role') not in entry['roles']: entry['roles'].append(meta.get('prompt_role'))
    lane=meta.get('evidence_lane')
    if lane and lane not in entry['evidence_lanes']: entry['evidence_lanes'].append(lane)
    for medium in meta.get('output_media') or []:
        if medium not in entry['media']: entry['media'].append(medium)
for entry in category_data['categories'].values():
    entry['prompt_ids'].sort(key=lambda value:int(value.split('-')[1]))
    entry['roles'].sort(); entry['evidence_lanes'].sort(); entry['media'].sort()
category_path.write_text(json.dumps(category_data,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')

# Rebuild template reverse routes and the prompt crosswalk from canonical prompts.
template_registry_path=ROOT/'template_registry.json'
...
for template in template_registry.get('templates',[]):
    ...
    template['required_by_prompt_ids']=sorted([m['prompt_id'] for m in rows if template_id in (m.get('template_routes') or [])],key=lambda value:int(value.split('-')[1]))
    template['conditional_by_prompt_ids']=sorted([m['prompt_id'] for m in rows if template_id in (m.get('conditional_template_routes') or [])],key=lambda value:int(value.split('-')[1]))
template_registry['suite_version']=VERSION
template_registry_path.write_text(json.dumps(template_registry,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')

template_crosswalk={'suite_version':VERSION,'mappings':[]}
for template in template_registry.get('templates',[]):
    prompt_ids=sorted(set(template.get('required_by_prompt_ids',[]))|set(template.get('conditional_by_prompt_ids',[])),key=lambda value:int(value.split('-')[1]))
    template_crosswalk['mappings'].append({'template_id':template['template_id'],'prompt_ids':prompt_ids,'path':template['path']})
template_crosswalk['mappings'].sort(key=lambda row:row['template_id'])
(ROOT/'integrations/template_to_prompt_crosswalk.json').write_text(json.dumps(template_crosswalk,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')

lines=[
    '# Prompt Catalog','',f'**Prompt count:** **{len(rows)}**','',
...
for m in rows:
    ...
    review='plan review → exact twin consent' if m.get('prompt_role')=='investigative' and m.get('paired_prompt_id') else 'reviewed handoff required' if m.get('prompt_role')=='executive' else '—'
    ...
    lines.append(f'| {m["sequence"]} | `{m["prompt_id"]}` | `{m["capability_id"]}` | [{m["title"]}]({m["canonical_path"]}) | `{m["prompt_role"]}` | `{m["category"]}` | `{evidence}` | `{m["risk_level"]}` | {modes} | {pair} | {review} | {templates} | {skills} |')
(ROOT/'PROMPT_CATALOG.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
print(json.dumps({'status':'pass','suite_version':VERSION,'prompt_count':len(rows)},indent=2))
