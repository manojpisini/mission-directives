...
def is_manifest_file(path:Path,root:Path)->bool:
    ...
    if path.is_symlink():
        raise ValueError(f'symbolic link is not allowed in sealed manifest: {relative.as_posix()}')
    if path.name in EXCLUDED_FILENAMES: return False
    if any(part in EXCLUDED_DIRNAMES for part in relative.parts): return False
    ...
    for directory,dirnames,filenames in os.walk(root,topdown=True,followlinks=False):
        ...
        for name in sorted(dirnames):
            ...
            if candidate.is_symlink():
                raise ValueError(f'symbolic link is not allowed in sealed manifest: {rel.as_posix()}')
        ...
        for name in sorted(filenames):
            path=current/name
            if is_manifest_file(path,root): yield path
...
def current(root:Path=ROOT)->dict[str,object]:
    ...
    return {'suite_version':(root/'VERSION').read_text(encoding='utf-8').strip(),'files':files}
...
def main()->int:
    ...
    if args.check:
        ...
        ok=old==now
        print(json.dumps({'status':'pass' if ok else 'fail','expected_files':len(now['files']),'manifest_files':len(old.get('files',[]))},indent=2)); return 0 if ok else 1
    atomic_write_json(path,now)
    print(len(now['files'])); return 0
