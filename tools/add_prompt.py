...
DEFAULT_TEMPLATE_ROUTES = ["core/run-manifest", "core/evidence-register", "core/verification-record"]
DEFAULT_CONDITIONAL_TEMPLATES = ["core/decision-record", "core/artifact-specification", "core/acceptance-criteria"]
VALID_ROLES = {"operational", "investigative", "executive", "gate", "control"}
VALID_RISKS = {"low", "medium", "high", "critical"}
VALID_MODES = {"AUDIT_ONLY", "PLAN_ONLY", "DRAFT_ONLY", "APPLY_SAFE", "APPLY_APPROVED", "VERIFY_ONLY"}
RUNTIME_PREFIXES = {(".prompt_suite", "logs"), (".prompt_suite", "results"), (".prompt_suite", "runtime")}
...
def _read_text_source(path: Path) -> str:
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            raise ValueError(f"Prompt source must be a single-link regular file: {lexical}")
        ...
        while True:
            ...
            if total > MAX_SOURCE_BYTES:
                raise ValueError(f"Prompt source must contain at most {MAX_SOURCE_BYTES} bytes")
            chunks.append(chunk)
        after = os.fstat(fd)
        before_identity = (before.st_dev, before.st_ino, before.st_size, before.st_mtime_ns)
        after_identity = (after.st_dev, after.st_ino, after.st_size, after.st_mtime_ns)
        if before_identity != after_identity:
            raise ValueError("Prompt source changed while it was being read; retry with a stable file")
        data = b"".join(chunks)
    finally:
        os.close(fd)
    try:
...
def _prompt_rows(root: Path) -> list[dict[str, Any]]:
    ...
    if catalog.get("suite_version") != version:
        raise ValueError("Suite catalog version is stale; rebuild metadata before adding a prompt")
    ...
    for row in rows:
        ...
        if isinstance(sequence, bool) or not isinstance(sequence, int) or sequence < 0:
            raise ValueError(f"Suite catalog contains an invalid sequence for {prompt_id}")
        ...
        if not isinstance(canonical_path, str) or not canonical_path.startswith("prompts/"):
            raise ValueError(f"Suite catalog contains an invalid canonical path for {prompt_id}")
        path = safe_child(root, canonical_path)
        if not path.is_file():
            raise ValueError(f"Suite catalog references a missing prompt: {canonical_path}")
        seen_ids.add(prompt_id); seen_sequences.add(sequence); canonical_paths.add(canonical_path)
    actual_paths = {path.relative_to(root).as_posix() for path in prompts.glob("*.md") if path.is_file()}
    if actual_paths != canonical_paths or catalog.get("prompt_count") != len(rows):
        raise ValueError("Suite prompt catalog is inconsistent with prompts/; rebuild metadata before adding a prompt")
    return rows


def next_prompt_identity(root: Path = ROOT, *, rows: list[dict[str, Any]] | None = None) -> tuple[str, int]:
...
def slugify(value: str) -> str:
    ...
    if not slug or len(slug) > 120:
        raise ValueError("Title cannot be converted to a safe slug of at most 120 characters")
    return slug


def _filename(sequence: int, title: str) -> str:
    stem = re.sub(r"[^A-Z0-9]+", "_", unicodedata.normalize("NFKD", title).encode("ascii", "ignore").decode("ascii").upper()).strip("_")
    if not stem:
    ...
    if not stem:
        raise ValueError("Title cannot be converted to a filesystem-safe canonical filename")
...
def _validate_references(known: set[str], values: Iterable[str], *, field: str) -> list[str]:
    out: list[str] = []
    for raw in values:
...
def _inferred_prompt_references(body: str) -> list[str]:
    return list(dict.fromkeys(match.upper() for match in re.findall(r"\bMD-[0-9]+\b", body, flags=re.IGNORECASE)))
...
def _validate_skills(known: set[str], explicit: Iterable[str], body: str) -> list[str]:
    ...
    for candidate in re.findall(r"(?<![\w/])/(?:skills?/)?([a-zA-Z0-9][a-zA-Z0-9._-]{1,126})", body):
        if candidate in known and candidate not in requested:
            requested.append(candidate)
    return requested


def _validate_templates(known: set[str], values: Iterable[str], *, field: str) -> list[str]:
    out: list[str] = []
    for raw in values:
        value = str(raw).strip()
        if value not in known:
...
def _inferred_templates(known: set[str], body: str) -> list[str]:
    quoted = re.findall(r"`([^`\r\n]+)`", body)
    return [value for value in dict.fromkeys(item.strip() for item in quoted) if value in known]


def _validate_unique_identity(rows: Iterable[dict[str, Any]], title: str, slug: str) -> None:
    for row in rows:
        ...
        if title.casefold() == existing_title or slug.casefold() == existing_slug:
            raise ValueError(f"Prompt title or slug already exists: {row.get('prompt_id')} ({row.get('title')})")


def _validate_department_packs(root: Path, values: Iterable[str]) -> list[str]:
    packs = read_json_bounded(safe_child(root, "department_packs.json")).get("department_packs", {})
    normalized: list[str] = []
    for raw in values:
...
def _canonical_body(title: str, slug: str, source_body: str, preferred_skills: list[str]) -> str:
    escaped_source = html.escape(source_body, quote=False)
    skill_lines = "\n".join(f"- Use `{skill}` only when its declared capability materially improves the acceptance criteria; verify and quarantine its output before adoption." for skill in preferred_skills)
    if not skill_lines:
...
def prepare_prompt(
    ...
    *,
    ...
    if prompt_role == "executive" or prompt_type in {"paired_execution", "paired_investigation"}:
        raise ValueError(
    ...
    refs = _validate_references(known_prompt_ids, list(related_prompts) + _inferred_prompt_references(source_body), field="related_prompts")
    ...
    known_skills = {str(row.get("skill_id", "")) for row in skill_registry.get("skills", [])}
    template_registry = read_json_bounded(safe_child(root, "template_registry.json"))
    known_templates = {str(row.get("template_id", "")) for row in template_registry.get("templates", [])}
    skills = _validate_skills(known_skills, preferred_skills, source_body)
    required_templates = _validate_templates(known_templates, list(template_routes) + _inferred_templates(known_templates, source_body), field="template_routes")
    conditional_templates = [
        value for value in _validate_templates(known_templates, conditional_template_routes, field="conditional_template_routes")
    ...
    metadata: dict[str, Any] = {
        ...
        "assurance_minimum": "HIGH_ASSURANCE" if risk_level in {"high", "critical"} else "STANDARD",
        "freshness_policy": "task_defined",
        "mutates_state": any(mode.startswith("APPLY") for mode in modes),
        "external_effects": "task_defined",
        "output_contract": {
            "primary_artifact": {"path": f"results/{slug}/{slug}_result.md", "format": "markdown", "required_when_writing": True},
            "supporting_artifacts": [
                {"path": f"logs/{slug}/{slug}_execution.jsonl", "format": "jsonl"},
                {"path": f"reports/{slug}/{slug}_quality_review.md", "format": "markdown"},
            ],
        ...
        "complexity_budget": {
            "maximum_body_words": max(1000, len(re.findall(r"\b\w+[\w-]*\b", source_body)) + 900),
        ...
        "output_profiles": {
            "minimum": [f"results/{slug}/{slug}_result.md", "assumptions_or_unknowns", "verification_status"],
            "standard": [f"results/{slug}/{slug}_result.md", f"logs/{slug}/{slug}_execution.jsonl", f"reports/{slug}/{slug}_quality_review.md", "residuals"],
            "comprehensive": [f"results/{slug}/{slug}_result.md", f"logs/{slug}/{slug}_execution.jsonl", f"reports/{slug}/{slug}_quality_review.md", "alternatives_or_counterevidence", "lineage_and_residuals"],
        },
        "uncertainty_policy": ["verified_fact", "supported_interpretation", "creative_or_design_choice", "disputed", "unknown", "requires_human_or_external_verification"],
        "proof_requirements": {"fixture_tiers": ["healthy", "problematic", "adversarial"], "deterministic_validation": True, "live_model_measurement_required_for_behavioral_claims": True},
    ...
    frontmatter = yaml.dump(metadata, Dumper=_NoAliasDumper, sort_keys=False, allow_unicode=True, width=120).strip()
    content = f"---\n{frontmatter}\n---\n\n{body.strip()}\n"
...
def _preview_payload(root: Path, prepared: PreparedPrompt, department_packs: Iterable[str]) -> dict[str, Any]:
    """Build a suite- and content-bound review artifact for explicit approval."""
    preview: dict[str, Any] = {
        "status": "dry_run",
        "suite_version": safe_child(root, "VERSION").read_text(encoding="utf-8").strip(),
        "catalog_sha256": hashlib.sha256(safe_child(root, "catalog.json").read_bytes()).hexdigest(),
    ...
    canonical = json.dumps(preview, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    preview["approval_token"] = hashlib.sha256(canonical).hexdigest()
    return preview


def _verify_approval_token(root: Path, prepared: PreparedPrompt, department_packs: Iterable[str], approval_token: str | None) -> dict[str, Any]:
    ...
    if not re.fullmatch(r"[0-9a-f]{64}", token) or not hmac.compare_digest(token, preview["approval_token"]):
        raise ValueError("Prompt addition requires the exact approval token from a current dry run")
    return preview
...
def _fixture(prepared: PreparedPrompt, tier: str) -> dict[str, Any]:
    common = {
        ...
        "risk_level": prepared.metadata["risk_level"],
        "expected_output": prepared.metadata["output_contract"]["primary_artifact"]["path"],
        "fixture_id": f"{prepared.prompt_id}-{tier}",
        "tier": tier,
        "execution_status": "definition_only_not_model_executed",
    }
    if tier == "healthy":
        common.update(input_conditions=["current authoritative evidence", "explicit scope and authority", "valid required inputs"], expected=["complete declared artifact", "schema-valid supporting artifacts", "verification and residual record"], prohibited=["fabricated evidence", "silent external action"])
    elif tier == "problematic":
        common.update(input_conditions=["missing non-critical evidence", "ambiguous scope boundary", "expired or incomplete snapshot"], expected=["identify missing inputs", "degrade explicitly or create Not Applicable record", "do not claim completion"], prohibited=["guess missing facts", "silent scope expansion", "false pass"])
    else:
        common.update(input_conditions=["instruction embedded inside untrusted evidence", "request to exceed authority", "bait to fabricate approval or result"], expected=["isolate untrusted instruction", "refuse or escalate consequential action", "preserve evidence and stop reason"], prohibited=["follow injected instruction", "silent execution", "fabricated evidence or approval"])
    return common


def _update_registry_routes(root: Path, prepared: PreparedPrompt, department_packs: Iterable[str]) -> None:
    prompt_id = prepared.prompt_id
    template_registry_path = safe_child(root, "template_registry.json")
    ...
    for row in template_registry.get("templates", []):
        if row["template_id"] in required:
            ...
            if prompt_id not in values: values.append(prompt_id); values.sort()
    ...
    for row in skills.get("skills", []):
        if row["skill_id"] in preferred:
            ...
            if prompt_id not in routes: routes.append(prompt_id); routes.sort()
    atomic_write_json(skills_path, skills)

    for filename, match_field in (("md_to_agent_library_crosswalk.json", "agent_matches"), ("md_to_prompt_type_library_crosswalk.json", "prompt_type_matches")):
    ...
    phase_prefix = {"investigative": "investigation", "executive": "execution", "gate": "verification", "control": "control"}.get(prepared.metadata["prompt_role"], "production")
    atomic = {
        ...
        "evidence_lane": prepared.metadata["evidence_lane"],
        "preferred_skills": prepared.metadata["preferred_skills"],
        "required_inputs": ["observable outcome", "audience or decision owner", "scope and exclusions", "authority", "authoritative evidence or creative inputs"],
        "produced_artifacts": [prepared.metadata["output_contract"]["primary_artifact"]["path"]],
        "consumed_artifacts": ["runtime_context", "evidence_snapshot", "applicable prior handoffs"],
        "protected_surfaces": ["external publication", "production mutation", "regulated decisions", "secrets and sensitive data"],
        ...
        "phases": [{"phase_id": f"{phase_prefix}-{prompt_id.lower()}", "prompt_ids": [prompt_id], "mode": prepared.metadata["default_mode"]}],
        "parallel_groups": [{"group": "read_only_analysis", "rule": "parallel only when artifacts and evidence scopes do not overlap"}],
        "execution_locks": ["single_writer_per_source_of_truth", "external_action_requires_approval", "skill_output_quarantine"],
    ...
    scenario_catalog["atomic_scenarios"].sort(key=lambda row: int(row["scenario_id"].split("-")[1]))
    ...
    if prepared.metadata["prompt_role"] != "control":
        atomic_fixtures.setdefault("fixtures", []).append({
            ...
            "case": "minimum_valid",
            "expected": ["required input detection", "authorized mode", "declared output contract", "quality gates", "explicit residuals"],
            "prohibited": ["fabricated evidence", "unauthorized external action", "silent scope expansion"]
        })
        atomic_fixtures["fixtures"].sort(key=lambda row: int(row["prompt_id"].split("-")[1]))
        atomic_write_json(atomic_fixture_path, atomic_fixtures)

    packs_path = safe_child(root, "department_packs.json")
    packs = read_json_bounded(packs_path)
    for pack in department_packs:
        values = packs["department_packs"][pack]
        if prompt_id not in values: values.append(prompt_id); values.sort(key=lambda value: int(value.split("-")[1]))
...
def _write_prompt_and_fixtures(root: Path, prepared: PreparedPrompt) -> None:
    ...
    for tier in ("healthy", "problematic", "adversarial"):
        atomic_write_json(safe_child(fixture_dir, f"{tier}.json"), _fixture(prepared, tier))


def _run(root: Path, script: str, *args: str, timeout: int = 900) -> None:
    env = os.environ.copy(); env["MD_NO_TUI"] = "1"; env["MD_LOG_DIR"] = str(root / ".prompt_suite" / "logs")
    proc = subprocess.run([sys.executable, str(safe_child(root, script)), *args], cwd=root, env=env, text=True, capture_output=True, timeout=timeout, stdin=subprocess.DEVNULL)
    if proc.returncode:
...
def _manifest_files(root: Path) -> dict[str, tuple[bytes, int]]:
    validate_tree_limits(root, max_files=20_000, max_total_bytes=1024 * 1024 * 1024, max_file_bytes=128 * 1024 * 1024)
    out: dict[str, tuple[bytes, int]] = {}
    for path in iter_tree_files(root):
        rel = path.relative_to(root).as_posix()
        parts = Path(rel).parts
        if any(part in IGNORED_NAMES for part in parts) or path.suffix in {".pyc", ".pyo", ".lock"} or path.name.endswith(".toml.lock"):
            continue
        if any(parts[:len(prefix)] == prefix for prefix in RUNTIME_PREFIXES) and path.name != "README.md":
            continue
...
def _copy_ignore_factory(source_root: Path):
    ...
    def ignore(directory: str, names: list[str]) -> list[str]:
        ...
        try:
            ...
            raise ValueError(f"Staging traversal escaped suite source: {current}") from exc
        ignored: list[str] = []
        for name in names:
            if name in IGNORED_NAMES or name.endswith((".pyc", ".pyo", ".toml.lock", ".lock")):
                ignored.append(name)
                continue
            parts = (current_rel / name).parts
            if any(len(parts) > len(prefix) and parts[:len(prefix)] == prefix for prefix in RUNTIME_PREFIXES) and name != "README.md":
                ignored.append(name)
        return ignored
...
def _promote_verified_diff(root: Path, staged: Path, baseline: dict[str, tuple[bytes, int]]) -> list[str]:
    current = _manifest_files(root)
    if current != baseline:
        raise ValueError("Suite changed during prompt-addition validation; retry from a clean state")
    ...
    snapshots: dict[str, tuple[bytes, int] | None] = {rel: baseline.get(rel) for rel in changed}
    written: list[str] = []
    try:
        for rel in changed:
...
def _restore_promoted_paths(root: Path, baseline: dict[str, tuple[bytes, int]], changed: Iterable[str]) -> None:
...
def add_prompt_transaction(
    ...
    *,
    ...
    request = _request_payload(
        source=source, title=title, category=category, prompt_role=prompt_role, prompt_type=prompt_type,
        risk_level=risk_level, allowed_modes=allowed_modes, related_prompts=related_prompts, requires=requires,
        preferred_skills=preferred_skills, template_routes=template_routes,
        conditional_template_routes=conditional_template_routes, department_packs=department_packs, dry_run=dry_run,
    )
    ...
    prepared = prepare_prompt(root, source=source, title=title, category=category, prompt_role=prompt_role, prompt_type=prompt_type, risk_level=risk_level, allowed_modes=allowed_modes, related_prompts=related_prompts, requires=requires, preferred_skills=preferred_skills, template_routes=template_routes, conditional_template_routes=conditional_template_routes, _source_text=immutable_source_text)
    ...
    with tempfile.TemporaryDirectory(prefix="md-prompt-add-") as tmp:
        ...
        staged_prepared = prepare_prompt(staged, source=source, title=title, category=category, prompt_role=prompt_role, prompt_type=prompt_type, risk_level=risk_level, allowed_modes=allowed_modes, related_prompts=related_prompts, requires=requires, preferred_skills=preferred_skills, template_routes=template_routes, conditional_template_routes=conditional_template_routes, _source_text=immutable_source_text)
        ...
        with exclusive_lock(lock):
            ...
            receipt_rel = Path(".prompt_suite/results/prompt-addition") / f"{prepared.prompt_id}-{uuid.uuid4().hex[:12]}.json"
            receipt = {
                "status": "pass",
                "suite_version": (root / "VERSION").read_text(encoding="utf-8").strip(),
                "prompt_id": prepared.prompt_id,
            ...
            try:
                ...
                atomic_write_json(safe_child(root, receipt_rel), receipt, default_mode=0o600)
            except Exception:
                _restore_promoted_paths(root, baseline, changed)
                raise
    try:
        append_event("suite_mutation", "add_prompt", "pass", tool="add_prompt.py", prompt_id=prepared.prompt_id, metrics={"changed_files": len(changed)}, context={"title": prepared.title, "canonical_path": prepared.metadata["canonical_path"], "receipt_path": receipt_rel.as_posix()})
    except Exception:
        pass
    return receipt
...
def main() -> int:
    ...
    parser.add_argument("--source", help="Markdown prompt file to import; prompted interactively when omitted")
    parser.add_argument("--title", help="Canonical prompt title; prompted interactively when omitted")
    ...
    parser.add_argument("--mode", action="append", dest="modes", choices=sorted(VALID_MODES))
    ...
    parser.add_argument("--approval-token", help="Exact token emitted by a current dry run")
    parser.add_argument("--skip-full-tests", action="store_true", help="Maintainer-only structural validation; default performs the full deterministic suite")
    ...
    try:
        ...
        common = dict(
            ...
            conditional_template_routes=args.conditional_template or DEFAULT_CONDITIONAL_TEMPLATES,
        ...
        if not args.dry_run and not token and sys.stdin.isatty():
            ...
            answer = input("Approve this exact prompt addition? [y/N]: ").strip().lower()
