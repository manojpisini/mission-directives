...
def _locked_source(lock_file: Path, skill_id: str, requested_source: str) -> str:
    ...
    if not lock or lock.get('lock_status') != 'resolved' or not lock.get('auto_install_allowed'):
    ...
    expected = validate_sha256(lock.get('tarball_sha256', ''), kind='locked tarball SHA-256')
...
def main() -> int:
    ...
    ap.add_argument('--acquisition-mode', choices=['locked_auto', 'approved_unpinned', 'manual_only'], default='locked_auto')
    ap.add_argument('--lock-file', default=str(ROOT / 'skills.lock.json'))
    ...
    source = _locked_source(Path(a.lock_file).expanduser(), skill_id, a.source) if a.acquisition_mode == 'locked_auto' else validate_github_repository(a.source)

    destination_roots = all_default_destinations()
    validate_distinct_roots(destination_roots.values())
    destinations = {name: safe_child(base, skill_id) for name, base in destination_roots.items()}
    ...
    try:
        append_event('skill', 'install_multi_location', 'started', tool='skill_dual.py', context={'skill_id': skill_id, 'mode': a.acquisition_mode, 'source_host': 'github.com'})
    except Exception:
        pass
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='md-skill-install-') as temp:
            ...
            cmd = [
                'npx', '--yes', SKILLS_CLI_PACKAGE, 'add', source, '--skill', skill_id,
                '-g', '-a', 'cline', '--copy', '-y',
            ]
            proc = subprocess.run(
                cmd, timeout=SKILLS_CLI_TIMEOUT_SECONDS, check=False, env=env,
                cwd=temp_root, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE,
                stderr=subprocess.PIPE, text=True,
            )
            if proc.returncode:
                raise RuntimeError(f'Skills CLI failed with exit code {proc.returncode}')
            staged_source = isolated_home / '.agents' / 'skills' / skill_id
            ensure_no_symlink_components(staged_source)
            if not staged_source.is_dir():
                raise RuntimeError('Skills CLI did not produce the expected isolated skill directory')
        ...
        tree_hashes = [tree_digest(destination) for destination in destinations.values()]
        if len(set(tree_hashes)) != 1:
            raise RuntimeError('Multi-location skill tree hashes differ')
        files = [safe_child(destination, 'SKILL.md') for destination in destinations.values()]
        ...
        try:
            append_event(
                'skill', 'install_multi_location', 'fail',
                duration_ms=int((dt.datetime.now(dt.timezone.utc) - started).total_seconds() * 1000),
                tool='skill_dual.py', context={'skill_id': skill_id}, error=str(exc),
            )
        except Exception:
            pass
        tui.finish('FAIL')
    ...
    try:
        event = append_event(
            'skill', 'install_multi_location', 'pass',
            duration_ms=int((dt.datetime.now(dt.timezone.utc) - started).total_seconds() * 1000),
            tool='skill_dual.py', context={'skill_id': skill_id, 'hash': result['skill_md_sha256']},
        )
