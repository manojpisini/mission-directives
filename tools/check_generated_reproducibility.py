...
def check(root: Path = ROOT) -> dict:
    ...
    if missing:
        return {"status": "fail", "missing": missing, "mismatches": []}
    with tempfile.TemporaryDirectory(prefix="md-repro-") as tmp:
        ...
        for script in GENERATORS:
            proc = subprocess.run(
                [sys.executable, str(clone / script)],
                cwd=clone,
                env=env,
                text=True,
                capture_output=True,
        ...
        for relative in GENERATED:
            a = (root / relative).read_bytes()
            b = (clone / relative).read_bytes()
            if a.replace(b"\r\n", b"\n") != b.replace(b"\r\n", b"\n"):
