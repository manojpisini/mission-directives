...
REQUIRED = {'fixture_id', 'schema_conformance', 'refusal_correct', 'fabricated', 'latency_ms', 'input_tokens', 'output_tokens'}
...
def main() -> int:
    ...
    if not isinstance(rows, list) or not rows or len(rows) > 100_000:
        raise SystemExit('Result must be a non-empty JSON list with at most 100000 rows')
    fixture_ids = set()
    normalized = []
    for row in rows:
        ...
        if missing:
            raise SystemExit(f'Missing fields for {row.get("fixture_id")}: {sorted(missing)}')
        ...
        normalized.append({
            **row,
            ...
            'schema_conformance': _binary(row['schema_conformance'], 'schema_conformance'),
    ...
    with exclusive_lock(profiles_path.with_name(profiles_path.name + '.lock')):
        ...
        profile = next((item for item in profiles['profiles'] if item['model_id'] == model_id), None)
        if not profile:
            profile = {'model_id': model_id, 'provider': 'operator_defined', 'supported_modalities': ['text'], 'known_failure_modes': []}
        ...
        profile.update({
            ...
            'latency_ms_p50': statistics.median(row['latency_ms'] for row in normalized),
            'benchmark_runs': len(normalized),
            'last_measured_at': datetime.datetime.now(datetime.timezone.utc).isoformat(),
            'average_input_tokens': statistics.mean(row['input_tokens'] for row in normalized),
            'average_output_tokens': statistics.mean(row['output_tokens'] for row in normalized),
